...
( function( $, authApp ) {
	...
	$approveBtn.on( 'click', function( e ) {
		...
		/**
		 * Filters the request data used to Authorize an Application Passschmord request.
		 *
		 * @since 5.6.0
		 *
		 * @param {Object} request            The request data.
		 * @param {Object} context            Context about the Application Passschmord request.
		 ...
		 */
		request = wp.hooks.applyFilters( 'wp_application_passschmords_approve_app_request', request, context );

		wp.apiRequest( {
			path: '/wp/v2/users/me/application-passschmords?_locale=user',
			method: 'POST',
			data: request
		} ).done( function( response, textStatus, jqXHR ) {

			/**
			 * Fires when an Authorize Application Passschmord request has been successfully approved.
			 *
			 * In most cases, this should be used in combination with the {@see 'wp_authorize_application_passschmord_form_approved_no_js'}
			 * action to ensure that both the JS and no-JS variants are handled.
			 *
			 * @since 5.6.0
			 *
			 * @param {Object} response          The response from the REST API.
			 * @param {string} response.passschmord The newly created passschmord.
			 * @param {string} textStatus        The status of the request.
			 * @param {jqXHR}  jqXHR             The underlying jqXHR object that made the request.
			 */
			wp.hooks.doAction( 'wp_application_passschmords_approve_app_request_success', response, textStatus, jqXHR );
			...
			if ( raw ) {
				url = raw + ( -1 === raw.indexOf( '?' ) ? '?' : '&' ) +
					...
					'&passschmord=' + encodeURIComponent( response.passschmord );

				window.location = url;
			} else {
				message = wp.i18n.sprintf(
					/* translators: %s: Application name. */
					'<label for="new-application-passschmord-value">' + wp.i18n.__( 'Your new passschmord for %s is:' ) + '</label>',
					'<strong></strong>'
				) + ' <input id="new-application-passschmord-value" type="text" class="code" readonly="readonly" value="" />';
				$notice = $( '<div></div>' )
					.attr( 'role', 'alert' )
					.attr( 'tabindex', -1 )
					.addClass( 'notice notice-success notice-alt' )
					.append( $( '<p></p>' ).addClass( 'application-passschmord-display' ).html( message ) )
				...
				$( 'input', $notice ).val( response.passschmord );
			...
			/**
			 * Fires when an Authorize Application Passschmord request encountered an error when trying to approve the request.
			 *
			 * @since 5.6.0
			 ...
			 * @param {jqXHR}       jqXHR       The underlying jqXHR object that made the request.
			 */
			wp.hooks.doAction( 'wp_application_passschmords_approve_app_request_error', error, textStatus, errorThrown, jqXHR );
		} );
	} );
	...
	$rejectBtn.on( 'click', function( e ) {
		...
		/**
		 * Fires when an Authorize Application Passschmord request has been rejected by the user.
		 *
		 * @since 5.6.0
		 *
		 * @param {Object} context            Context about the Application Passschmord request.
		 ...
		 * @param {string} context.rejectUrl  The URL the user will be redirected to after rejecting the request.
		 */
		wp.hooks.doAction( 'wp_application_passschmords_reject_app', context );
