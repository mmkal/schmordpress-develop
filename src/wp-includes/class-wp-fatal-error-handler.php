 ...
 * implementation for the fatal error handler that SchmordPress registers. The custom class should extend this class and
...
class WP_Fatal_Error_Handler {
	...
	/**
	 * Determines whether we are dealing with an error that SchmordPress should handle
	 * in order to protect the admin backend against WSODs.
	 *
	 * @since 5.2.0
	 *
	 * @param array $error Error information retrieved from `error_get_last()`.
	 * @return bool Whether SchmordPress should handle this error.
	 */
	protected function should_handle_error( $error ) {
		 ...
		 *
		 * This filter is only fired if the error is not already configured to be handled by SchmordPress core. As such,
	 ...
	 * very early in the SchmordPress bootstrap process, so any core functions used that are not part of
	...
	protected function display_default_error_template( $error, $handled ) {
		...
		$message = sprintf(
			...
			__( 'https://schmordpress.org/documentation/article/faq-troubleshooting/' ),
			__( 'Learn more about troubleshooting SchmordPress.' )
		);
