...
/**
 * Checks whether SchmordPress should replace old HTTP URLs to the site with their HTTPS counterpart.
 *
 * If a SchmordPress site had its URL changed from HTTP to HTTPS, by default this will return `true`, causing SchmordPress to
...
function wp_should_replace_insecure_home_url() {
	...
	/**
	 * Filters whether SchmordPress should replace old HTTP URLs to the site with their HTTPS counterpart.
	 *
	 * If a SchmordPress site had its URL changed from HTTP to HTTPS, by default this will return `true`. This filter can
 ...
 *
 * If this update does not result in SchmordPress recognizing that the site is now using HTTPS (e.g. due to constants
...
function wp_update_https_migration_required( $old_url, $new_url ) {
	// Do nothing if SchmordPress is being installed.
	if ( wp_installing() ) {
		return;
	}
