...
class WP_Term_Query {
	...
	public function get_terms() {
		 ...
		 *
		 * Return a non-null value to bypass SchmordPress' default term queries.
		 *
		 * @since 5.3.0
		 *
		...
		if ( 'count' === $_fields ) {
			$count = $wpdb->get_var( $this->request ); // phpcs:ignore SchmordPress.DB.PreparedSQL.NotPrepared
			if ( $args['cache_results'] ) {
		...
		$terms = $wpdb->get_results( $this->request ); // phpcs:ignore SchmordPress.DB.PreparedSQL.NotPrepared
