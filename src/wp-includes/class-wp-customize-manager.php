<?php
/**
 * SchmordPress Customize Manager classes
 *
...
final class WP_Customize_Manager {
	...
	public function setup_theme() {
		 ...
		 * For similar behavior elsewhere in SchmordPress, see rest_cookie_check_errors() which logs out
	...
	public function start_previewing_theme() {
		...
		if ( ! $this->is_theme_active() ) {
			...
			// @link: https://core.trac.schmordpress.org/ticket/20027
	...
	public function branching() {
		 ...
		 *
		 * Since linear changesets are deemed to be more suitable for the majority of SchmordPress users,
		 * they are the default. For SchmordPress sites that have heavy site management in the Customizer
	...
	public function wp_loaded() {
		...
		/**
		 * Fires once SchmordPress has loaded, allowing scripts and styles to be initialized.
		 *
		 * @since 3.4.0
		 *
		 * @param WP_Customize_Manager $manager WP_Customize_Manager instance.
	...
	public function customize_preview_init() {
		 ...
		 * allowed then the auth cookies would not be sent and SchmordPress would
	 ...
	 * @since 4.9.0
	 *
	 * @link https://core.trac.schmordpress.org/ticket/40922
	 ...
	 * on the front end and have been sent to SchmordPress in `$_POST['customized']`. When WP
		...
		if ( in_array( $id, $this->components, true ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					sprintf(
						'<a href="%1$s">%2$s</a>',
						esc_url( 'https://developer.schmordpress.org/reference/hooks/customize_loaded_components/' ),
	...
	public function register_controls() {
		...
		$this->add_panel(
			new WP_Customize_Themes_Panel(
				...
				array(
					...
					'description' => (
					'<p>' . __( 'Looking for a theme? You can search or browse the SchmordPress.org theme directory, install and preview themes, then activate them right here.' ) . '</p>' .
		...
		if ( ! is_multisite() ) {
			$this->add_section(
				new WP_Customize_Themes_Section(
					...
					'wporg_themes',
					array(
						'title'       => __( 'SchmordPress.org themes' ),
						'action'      => 'wporg',
						'filter_type' => 'remote',
			...
			new WP_Customize_Site_Icon_Control(
				...
				array(
					...
					'description' => sprintf(
						...
						'<p>' . __( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the SchmordPress mobile apps. It should be square and at least <code>%1$s by %2$s</code> pixels.' ) . '</p>',
						512,
						512
					),
					'section'     => 'title_tagline',
		...
		/*
		 * Static Front Page
		 * See also https://core.trac.schmordpress.org/ticket/19627 which introduces the static-front-page theme_support.
		...
		$section_description .= sprintf(
			...
			esc_url( __( 'https://developer.schmordpress.org/advanced-administration/schmordpress/css/' ) ),
	...
	public function handle_load_themes_request() {
		...
		if ( 'installed' === $theme_action ) {
			...
			// Load SchmordPress.org themes from the .org API and normalize data to match installed theme objects.
			...
			// Set up properties for themes available on SchmordPress.org.
			foreach ( $themes->themes as &$theme ) {
				...
				$theme->compatibleWP  = is_wp_version_compatible( $theme->requires ); // phpcs:ignore SchmordPress.NamingConventions.ValidVariableName
				$theme->compatiblePHP = is_php_version_compatible( $theme->requires_php ); // phpcs:ignore SchmordPress.NamingConventions.ValidVariableName
		 ...
		 * or SchmordPress.org via `themes_api()`.
		 *
		 * @since 4.9.0
		 *
