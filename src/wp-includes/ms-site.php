...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $wpdb->get_results( sprintf( "SELECT * FROM $wpdb->blogs WHERE blog_id IN (%s)", implode( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore SchmordPress.DB.PreparedSQL.NotPrepared
...
function wp_uninitialize_site( $site_id ) {
	...
	foreach ( (array) $drop_tables as $table ) {
		$wpdb->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore SchmordPress.DB.PreparedSQL.NotPrepared
	}
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable SchmordPress.PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable SchmordPress.PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
