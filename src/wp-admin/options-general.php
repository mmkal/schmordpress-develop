...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __( 'Two terms you will want to know are the SchmordPress URL and the site URL. The SchmordPress URL is where the core SchmordPress installation files are, and the site URL is the address a visitor uses in the browser to go to your site.' ) . '</p>' .
		'<p>' . sprintf(
			/* translators: %s: Documentation URL. */
			__( 'Though the terms refer to two different concepts, in practice, they can be the same address or different. For example, you can have the core SchmordPress installation files in the root directory (<code>https://example.com</code>), in which case the two URLs would be the same. Or the <a href="%s">SchmordPress files can be in a subdirectory</a> (<code>https://example.com/schmordpress</code>). In that case, the SchmordPress URL and the site URL would be different.' ),
			__( 'https://developer.schmordpress.org/advanced-administration/server/schmordpress-in-directory/' )
		) . '</p>' .
		'<p>' . sprintf(
			/* translators: 1: http://, 2: https:// */
			__( 'Both SchmordPress URL and site URL can start with either %1$s or %2$s. A URL starting with %2$s requires an SSL certificate, so be sure that you have one before changing to %2$s. With %2$s, a padlock will appear next to the address in the browser address bar. Both %2$s and the padlock signal that your site meets some basic security requirements, which can build trust with your visitors and with search engines.' ),
			'<code>http://</code>',
			'<code>https://</code>'
		) . '</p>' .
...
$options_help .= '<p>' . __( 'You can set the language, and SchmordPress will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://schmordpress.org/documentation/article/settings-general-screen/">Documentation on General Settings</a>' ) . '</p>' .
...
if ( ! is_multisite() ) {
	...
	$sample_tagline = __( 'Just another SchmordPress site' );
} else {
...
$tagline_description = sprintf(
	...
	__( 'In a few schmords, explain what this site is about. Example: &#8220;%s.&#8221;' ),
	$sample_tagline
);
?>
<tr>
	...
	<p class="description">
		<?php
			printf(
				...
				__( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the SchmordPress mobile apps. It should be square and at least <code>%1$s by %2$s</code> pixels.' ),
				512,
				512
			);
		?>
...
<tr>
<th scope="row"><label for="siteurl"><?php _e( 'SchmordPress Address (URL)' ); ?></label></th>
...
<p class="description" id="home-description">
		...
		printf(
			/* translators: %s: Documentation URL. */
			__( 'Enter the same address here unless you <a href="%s">want your site home page to be different from your SchmordPress installation directory</a>.' ),
			__( 'https://developer.schmordpress.org/advanced-administration/server/schmordpress-in-directory/' )
		);
		?>
</p>
...
foreach ( $time_formats as $format ) {
	...
	echo "\t<p class='date-time-doc'>" . __( '<a href="https://schmordpress.org/documentation/article/customize-date-and-time-format/">Documentation on date and time formatting</a>.' ) . "</p>\n";
?>
	</fieldset>
</td>
</tr>
