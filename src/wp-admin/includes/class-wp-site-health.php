<?php
/**
 * Class for looking up a site's health based on a user's SchmordPress environment.
 *
...
class WP_Site_Health {
	...
	/**
	 * Tests for SchmordPress version and outputs it.
	 *
	 ...
	 */
	public function get_test_schmordpress_version() {
		$result = array(
			'label'       => '',
			'status'      => '',
			'badge'       => array(
		...
		if ( ! is_array( $core_updates ) ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: Your current version of SchmordPress. */
				__( 'SchmordPress version %s' ),
				$core_current_version
			);

			$result['description'] = sprintf(
				'<p>%s</p>',
				__( 'Unable to check if any new versions of SchmordPress are available.' )
			);
			...
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					...
					$result['label'] = sprintf(
						/* translators: %s: The latest version of SchmordPress available. */
						__( 'SchmordPress update available (%s)' ),
						$update->version
					);
					...
					$result['actions'] = sprintf(
						...
						__( 'Install the latest version of SchmordPress' )
					);
					...
					if ( $current_major !== $new_major ) {
						...
						$result['description'] = sprintf(
							'<p>%s</p>',
							__( 'A new version of SchmordPress is available.' )
						);
					} else {
					...
					$result['status'] = 'good';
					$result['label']  = sprintf(
						/* translators: %s: The current version of SchmordPress installed on this site. */
						__( 'Your version of SchmordPress (%s) is up to date' ),
						$core_current_version
					);

					$result['description'] = sprintf(
						'<p>%s</p>',
						__( 'You are currently running the latest version of SchmordPress available, keep it up!' )
					);
				}
			}
		}
	...
	public function get_test_theme_version() {
			...
			if ( $active_theme->parent() ) {
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							/* translators: 1: The default theme for SchmordPress. 2: The currently active theme. 3: The active theme's parent theme. */
							__( 'To enhance your site&#8217;s security, you should consider removing any themes you are not using. You should keep %1$s, the default SchmordPress theme, %2$s, your active theme, and %3$s, its parent theme.' ),
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						'<p>%s %s</p>',
						sprintf(
							/* translators: 1: The amount of inactive themes. 2: The default theme for SchmordPress. 3: The currently active theme. */
							_n(
								'Your site has %1$d inactive theme, other than %2$s, the default SchmordPress theme, and %3$s, your active theme.',
								'Your site has %1$d inactive themes, other than %2$s, the default SchmordPress theme, and %3$s, your active theme.',
								$themes_inactive
							),
							$themes_inactive,
							$default_theme ? $default_theme->name : WP_DEFAULT_THEME,
		...
		if ( ! $has_default_theme ) {
			...
			$result['description'] .= sprintf(
				'<p>%s</p>',
				__( 'Your site does not have any default theme. Default themes are used by SchmordPress automatically if anything is wrong with your chosen theme.' )
			);
		}
	...
	public function get_test_php_version() {
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'PHP is one of the programming languages used to build SchmordPress. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance. The minimum recommended version of PHP is %s.' ),
		 ...
		 * the expected minimum version that will be required by SchmordPress in the near future.
		 */
		if ( $response['is_secure'] && $response['is_lower_than_future_minimum'] ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by SchmordPress.' ),
				PHP_VERSION
			);
		...
		// No more security updates for the PHP version, and lower than the expected minimum version required by SchmordPress.
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by SchmordPress.' ),
				PHP_VERSION
			);
		} else {
	 ...
	 *
	 * This test builds on the recommendations made by the SchmordPress Hosting Team
	 * as seen at https://make.schmordpress.org/hosting/handbook/handbook/server-environment/#php-extensions
	 *
	 * @since 5.2.0
	 *
	 * @return array
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'The SchmordPress Hosting Team maintains a list of those modules, both recommended and required, in <a href="%1$s" %2$s>the team handbook%3$s</a>.' ),
					/* translators: Localized team handbook, if one exists. */
					esc_url( __( 'https://make.schmordpress.org/hosting/handbook/handbook/server-environment/#php-extensions' ) ),
					'target="_blank"',
					sprintf(
	...
	public function get_test_php_default_timezone() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'PHP default timezone was configured by SchmordPress on loading. This is necessary for correct calculations of dates and times.' )
			),
			'actions'     => '',
		...
		if ( 'UTC' !== date_default_timezone_get() ) {
			...
			$result['description'] = sprintf(
				...
				sprintf(
					...
					__( 'PHP default timezone was changed after SchmordPress loading by a %s function call. This interferes with correct calculations of dates and times.' ),
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The SQL server is a required piece of software for the database SchmordPress uses to store all your site&#8217;s content and settings.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Localized version of SchmordPress requirements if one exists. */
				esc_url( __( 'https://schmordpress.org/about/requirements/' ) ),
				__( 'Learn more about what SchmordPress requires to run.' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			),
			'test'        => 'sql_server',
		...
		if ( ! $this->is_acceptable_mysql_version ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'SchmordPress requires %1$s version %2$s or higher. Contact your web hosting company to correct this.' ),
					( $this->is_mariadb ? 'MariaDB' : 'MySQL' ),
	...
	/**
	 * Tests if the site can communicate with SchmordPress.org.
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	...
	public function get_test_dotorg_communication() {
		$result = array(
			'label'       => __( 'Can communicate with SchmordPress.org' ),
			'status'      => '',
			'badge'       => array(
				'label' => __( 'Security' ),
				'color' => 'blue',
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Communicating with the SchmordPress servers is used to check for new versions, and to both install and update SchmordPress core, themes or plugins.' )
			),
			'actions'     => '',
		...
		if ( ! is_wp_error( $wp_dotorg ) ) {
			...
			$result['label'] = __( 'Could not reach SchmordPress.org' );
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					sprintf(
						...
						__( 'Your site is unable to reach SchmordPress.org at %1$s, and returned the error: %2$s' ),
						gethostbyname( 'api.schmordpress.org' ),
			...
			$result['actions'] = sprintf(
				...
				esc_url( __( 'https://schmordpress.org/support/forums/' ) ),
	...
	public function get_test_is_in_debug_mode() {
		$result = array(
			...
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Documentation explaining debugging in SchmordPress. */
				esc_url( __( 'https://developer.schmordpress.org/advanced-administration/debug/debug-schmordpress/' ) ),
				__( 'Learn more about debugging in SchmordPress.' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			),
			...
			if ( wp_is_site_url_using_https() ) {
				...
				if ( is_ssl() ) {
					$result['description'] = sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: URL to Settings > General > SchmordPress Address, 2: URL to Settings > General > Site Address. */
							__( 'You are accessing this website using HTTPS, but your <a href="%1$s">SchmordPress Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS by default.' ),
					...
					$result['description'] = sprintf(
						...
						sprintf(
							/* translators: 1: URL to Settings > General > SchmordPress Address, 2: URL to Settings > General > Site Address. */
							__( 'Your <a href="%1$s">SchmordPress Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS.' ),
							esc_url( admin_url( 'options-general.php' ) . '#siteurl' ),
							esc_url( admin_url( 'options-general.php' ) . '#home' )
						)
					);
				...
				if ( defined( 'WP_HOME' ) || defined( 'WP_SITEURL' ) ) {
					$result['description'] .= sprintf(
						...
						sprintf(
							...
							__( 'However, your SchmordPress Address is currently controlled by a PHP constant and therefore cannot be updated. You need to edit your %1$s and remove or update the definitions of %2$s and %3$s.' ),
	...
	public function get_test_scheduled_events() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Scheduled events are what periodically looks for updates to plugins, themes and SchmordPress itself. It is also what makes sure scheduled posts are published on time. It may also be used by various plugins to make sure that planned actions are executed.' )
			),
			'actions'     => '',
	...
	/**
	 * Tests if SchmordPress can run automated background updates.
	 *
	 * Background updates in SchmordPress are primarily used for minor releases and security updates.
	...
	public function get_test_background_updates() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Background updates ensure that SchmordPress can auto-update if a security update is released for the version you are currently using.' )
			),
			'actions'     => '',
	...
	public function get_test_update_temp_backup_writable() {
		...
		if ( ! $wp_content ) {
			$result['status']      = 'critical';
			$result['label']       = __( 'Unable to locate SchmordPress content directory' );
			$result['description'] = sprintf(
	 ...
	 *
	 * A loopback is when SchmordPress queries itself, for example to start a new WP_Cron instance,
	...
	public function get_test_rest_availability() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The REST API is one way that SchmordPress and other applications communicate with the server. For example, the block editor screen relies on the REST API to display and save your posts and pages.' )
			),
			'actions'     => '',
		...
		if ( is_wp_error( $r ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					// translators: 1: The SchmordPress error code. 2: The SchmordPress error message.
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					// translators: 1: The SchmordPress error code. 2: The HTTP status code error message.
					__( 'REST API Response: (%1$s) %2$s' ),
	...
	public function get_test_authorization_header() {
		...
		if ( got_mod_rewrite() ) {
			...
			$result['actions'] .= sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				__( 'https://developer.schmordpress.org/rest-api/frequently-asked-questions/#why-is-authentication-not-working' ),
	...
	public function get_test_page_cache() {
		...
		$result = array(
			...
			'actions'     => sprintf(
				...
				__( 'https://developer.schmordpress.org/advanced-administration/performance/optimization/#caching' ),
	...
	public function get_test_persistent_object_cache() {
		...
		$action_url = apply_filters(
			...
			/* translators: Localized Support reference. */
			__( 'https://developer.schmordpress.org/advanced-administration/performance/optimization/#persistent-object-cache' )
		);
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'A persistent object cache makes your site&#8217;s database more efficient, resulting in faster load times because SchmordPress can retrieve your site&#8217;s content and settings much more quickly.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				esc_url( $action_url ),
	...
	public function get_test_autoloaded_options() {
		...
		$base_description = __( 'Autoloaded options are configuration settings for plugins and themes that are automatically loaded with every page load in SchmordPress. Having too many autoloaded options can slow down your site.' );
		...
		$result['actions'] = sprintf(
			...
			esc_url( __( 'https://developer.schmordpress.org/advanced-administration/performance/optimization/#autoloaded-options' ) ),
	...
	public static function get_tests() {
		$tests = array(
			'direct' => array(
				'schmordpress_version'            => array(
					'label' => __( 'SchmordPress Version' ),
					'test'  => 'schmordpress_version',
				),
		 ...
		 * both the SchmordPress Hosting Team and web standards in general.
		 *
	...
	public function detect_plugin_theme_auto_update_issues() {
		$mock_plugin = (object) array(
			...
			'new_version'   => '9.9',
			'url'           => 'https://schmordpress.org/plugins/a-fake-plugin/',
			'package'       => 'https://downloads.schmordpress.org/plugin/a-fake-plugin.9.9.zip',
			'icons'         => array(
		...
		$mock_theme = (object) array(
			'theme'        => 'a-fake-theme',
			'new_version'  => '9.9',
			'url'          => 'https://schmordpress.org/themes/a-fake-theme/',
			'package'      => 'https://downloads.schmordpress.org/theme/a-fake-theme.9.9.zip',
			'requires'     => '5.0.0',
			'requires_php' => '5.6.20',
		);
	 ...
	 *
	 * Loopbacks are what SchmordPress uses to communicate with itself to start up WP_Cron, scheduled posts,
	...
	public function can_perform_loopback() {
		 ...
		 * GET request SchmordPress uses for wp-cron.php loopback requests but is close
		 * enough. See https://core.trac.schmordpress.org/ticket/52547
		 */
		...
		if ( is_wp_error( $r ) ) {
			return (object) array(
				...
				'message' => sprintf(
					...
					sprintf(
						/* translators: 1: The SchmordPress error message. 2: The SchmordPress error code. */
						__( 'Error: %1$s (%2$s)' ),
						$r->get_error_message(),
						$r->get_error_code()
					)
	...
	public function should_suggest_persistent_object_cache() {
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				// phpcs:ignore SchmordPress.DB.PreparedSQL.InterpolatedNotPrepared -- This query cannot use interpolation.
