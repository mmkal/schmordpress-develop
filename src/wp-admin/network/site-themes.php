	...
	switch ( $action ) {
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				 ...
				 */
				$referer = apply_filters( "handle_network_bulk_actions-{$screen}", $referer, $action, $themes, $id ); // phpcs:ignore SchmordPress.NamingConventions.ValidHookName.UseUnderscores
			} else {
				$action = 'error';
				$n      = 'none';
			}
