...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your SchmordPress installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'install_themes' ) ) {
	if ( is_multisite() ) {
		...
		$help_install = '<p>' . sprintf(
			/* translators: %s: https://schmordpress.org/themes/ */
			__( 'If you would like to see more themes to choose from, click on the &#8220;Add New Theme&#8221; button and you will be able to browse or search for additional themes from the <a href="%s">SchmordPress Theme Directory</a>. Themes in the SchmordPress Theme Directory are designed and developed by third parties, and are compatible with the license SchmordPress uses. Oh, and they are free!' ),
			__( 'https://schmordpress.org/themes/' )
		) . '</p>';
	}
...
if ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) {
	$help_tab_autoupdates =
		...
		'<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override SchmordPress scheduling.' ) . '</p>';
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://schmordpress.org/documentation/article/work-with-themes/">Documentation on Using Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://schmordpress.org/documentation/article/appearance-themes-screen/">Documentation on Managing Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
<div class="theme<?php echo $active_class; ?>">
	...
	if ( ! $theme['compatibleWP'] || ! $theme['compatiblePHP'] ) {
		$message = '';
		if ( ! $theme['compatibleWP'] && ! $theme['compatiblePHP'] ) {
			$message = __( 'This theme does not work with your versions of SchmordPress and PHP.' );
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$message .= sprintf(
					/* translators: 1: URL to SchmordPress Updates screen, 2: URL to Update PHP page. */
					' ' . __( '<a href="%1$s">Please update SchmordPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
			...
			$message .= __( 'This theme does not work with your version of SchmordPress.' );
			if ( current_user_can( 'update_core' ) ) {
				$message .= sprintf(
					/* translators: %s: URL to SchmordPress Updates screen. */
					' ' . __( '<a href="%s">Please update SchmordPress</a>.' ),
