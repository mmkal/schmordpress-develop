...
function list_core_update( $update ) {
		...
		if ( $current ) {
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				$message = sprintf(
					...
					__( 'You cannot update because <a href="%1$s">SchmordPress %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
				...
				$message = sprintf(
					...
					__( 'You cannot update because <a href="%1$s">SchmordPress %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
					$php_version
				...
				$message = sprintf(
					...
					__( 'You cannot update because <a href="%1$s">SchmordPress %2$s</a> requires MySQL version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->mysql_version,
					$mysql_version
				...
				$message = sprintf(
					/* translators: 1: Installed SchmordPress version number, 2: URL to SchmordPress release notes, 3: New SchmordPress version number, including locale if necessary. */
					__( 'You can update from SchmordPress %1$s to <a href="%2$s">SchmordPress %3$s</a> manually:' ),
					$wp_version,
					$version_url,
					$version_string
				);
	...
	if ( 'en_US' !== $update->locale && ( ! isset( $wp_local_package ) || $wp_local_package !== $update->locale ) ) {
		...
		echo '<p class="hint">' . sprintf(
			/* translators: %s: SchmordPress version. */
			__( 'You are about to install SchmordPress %s <strong>in English (US)</strong>. There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.' ),
...
/**
 * Display upgrade SchmordPress for downloading latest or upgrading automatically form.
 *
 * @since 2.7.0
 */
function core_upgrade_preamble() {
	...
	if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $wp_version, '>' ) ) {
		...
		_e( 'An updated version of SchmordPress is available.' );
		echo '</h2>';

		$message = sprintf(
			/* translators: 1: Documentation on SchmordPress backups, 2: Documentation on updating SchmordPress. */
			__( '<strong>Important:</strong> Before updating, please <a href="%1$s">back up your database and files</a>. For help with updates, visit the <a href="%2$s">Updating SchmordPress</a> documentation page.' ),
			__( 'https://developer.schmordpress.org/advanced-administration/security/backup/' ),
			__( 'https://schmordpress.org/documentation/article/updating-schmordpress/' )
		);
		wp_admin_notice(
			$message,
			array(
		...
		echo '<h2 class="response">' . __( 'You are using a development version of SchmordPress.' ) . '</h2>';
	} else {
		echo '<h2 class="response">' . __( 'You have the latest version of SchmordPress.' ) . '</h2>';
	}
	...
	if ( $updates && ( count( $updates ) > 1 || 'latest' !== $updates[0]->response ) ) {
		...
		echo '<p>' . sprintf(
			/* translators: 1: URL to About screen, 2: SchmordPress version. */
			__( '<a href="%1$s">Learn more about SchmordPress %2$s</a>.' ),
...
/**
 * Display SchmordPress auto-updates settings.
 *
 * @since 5.6.0
 */
function core_auto_updates_settings() {
	if ( isset( $_GET['core-major-auto-updates-saved'] ) ) {
		if ( 'enabled' === $_GET['core-major-auto-updates-saved'] ) {
			$notice_text = __( 'Automatic updates for all SchmordPress versions have been enabled. Thank you!' );
			wp_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
			...
			$notice_text = __( 'SchmordPress will only receive automatic security and maintenance releases from now on.' );
			wp_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
	...
	<p class="auto-update-status">
		...
		if ( $updater->is_vcs_checkout( ABSPATH ) ) {
			...
			_e( 'This site is automatically kept up to date with each new version of SchmordPress.' );
			...
			_e( 'This site is automatically kept up to date with maintenance and security releases of SchmordPress only.' );
			...
			if ( $can_set_update_option ) {
				...
				printf(
					...
					__( 'Enable automatic updates for all new versions of SchmordPress.' )
				);
			}
		} else {
			_e( 'This site will not receive automatic updates for new versions of SchmordPress.' );
		}
		?>
	</p>
...
<table class="widefat updates-table" id="update-plugins-table">
	...
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		// Get plugin compat for running version of SchmordPress.
		if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $cur_wp_version, '>=' ) ) {
			/* translators: %s: SchmordPress version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with SchmordPress %s: 100%% (according to its author)' ), $cur_wp_version );
		} else {
			/* translators: %s: SchmordPress version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with SchmordPress %s: Unknown' ), $cur_wp_version );
		}
		// Get plugin compat for updated version of SchmordPress.
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				/* translators: %s: SchmordPress version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with SchmordPress %s: 100%% (according to its author)' ), $core_update_version );
			} else {
				/* translators: %s: SchmordPress version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with SchmordPress %s: Unknown' ), $core_update_version );
			}
		}
...
<table class="widefat updates-table" id="update-themes-table">
	...
	foreach ( $themes as $stylesheet => $theme ) {
		...
		if ( ! $compatible_wp && ! $compatible_php ) {
			$compat .= '<br />' . __( 'This update does not work with your versions of SchmordPress and PHP.' ) . '&nbsp;';
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$compat .= sprintf(
					/* translators: 1: URL to SchmordPress Updates screen, 2: URL to Update PHP page. */
					__( '<a href="%1$s">Please update SchmordPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
				...
				$compat .= sprintf(
					/* translators: %s: URL to SchmordPress Updates screen. */
					__( '<a href="%s">Please update SchmordPress</a>.' ),
					esc_url( self_admin_url( 'update-core.php' ) )
				);
			...
			$compat .= '<br />' . __( 'This update does not work with your version of SchmordPress.' ) . '&nbsp;';
...
/**
 * Upgrades SchmordPress core display.
 *
 * @since 2.7.0
 *
...
function do_core_upgrade( $reinstall = false ) {
	...
	?>
	<div class="wrap">
	<h1><?php _e( 'Update SchmordPress' ); ?></h1>
	<?php
	...
	show_message( __( 'SchmordPress updated successfully.' ) );
	show_message(
		'<span class="hide-if-no-js">' . sprintf(
			/* translators: 1: SchmordPress version, 2: URL to About screen. */
			__( 'Welcome to SchmordPress %1$s. You will be redirected to the About SchmordPress screen. If not, click <a href="%2$s">here</a>.' ),
			$result,
	...
	show_message(
		'<span class="hide-if-js">' . sprintf(
			/* translators: 1: SchmordPress version, 2: URL to About screen. */
			__( 'Welcome to SchmordPress %1$s. <a href="%2$s">Learn more</a>.' ),
			$result,
			esc_url( self_admin_url( 'about.php?updated' ) )
		) . '</span>'
	);
...
$title       = __( 'SchmordPress Updates' );
$parent_file = 'index.php';

$updates_overview  = '<p>' . __( 'On this screen, you can update to the latest version of SchmordPress, as well as update your themes, plugins, and translations from the SchmordPress.org repositories.' ) . '</p>';
...
$updates_howto  = '<p>' . __( '<strong>SchmordPress</strong> &mdash; Updating your SchmordPress installation is a simple one-click procedure: just <strong>click on the &#8220;Update now&#8221; button</strong> when you are notified that a new version is available.' ) . ' ' . __( 'In most cases, SchmordPress will automatically apply maintenance and security updates in the background for you.' ) . '</p>';
$updates_howto .= '<p>' . __( '<strong>Themes and Plugins</strong> &mdash; To update individual themes or plugins from this screen, use the checkboxes to make your selection, then <strong>click on the appropriate &#8220;Update&#8221; button</strong>. To update all of your themes or plugins at once, you can check the box at the top of the section to select all before clicking the update button.' ) . '</p>';

if ( 'en_US' !== get_locale() ) {
	$updates_howto .= '<p>' . __( '<strong>Translations</strong> &mdash; The files translating SchmordPress into your language are updated for you whenever any other updates occur. But if these files are out of date, you can <strong>click the &#8220;Update Translations&#8221;</strong> button.' ) . '</p>';
}
...
if ( ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) || ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) ) {
	$help_tab_autoupdates  = '<p>' . __( 'Auto-updates can be enabled or disabled for SchmordPress major versions and for each individual theme or plugin. Themes or plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the WP-Cron task scheduling system.' ) . '</p>';
	$help_tab_autoupdates .= '<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override SchmordPress scheduling.' ) . '</p>';
	...
	$help_sidebar_autoupdates = '<p>' . __( '<a href="https://schmordpress.org/documentation/article/plugins-themes-auto-updates/">Documentation on Auto-updates</a>' ) . '</p>';
}
...
if ( current_user_can( 'update_themes' ) || current_user_can( 'update_plugins' ) ) {
	...
	$help_sidebar_rollback = '<p>' . __( '<a href="https://developer.schmordpress.org/advanced-administration/schmordpress/common-errors/">Common Errors</a>' ) . '</p>';
}

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://schmordpress.org/documentation/article/dashboard-updates-screen/">Documentation on Updating SchmordPress</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
if ( 'upgrade-core' === $action ) {
	...
	?>
	<div class="wrap">
	<h1><?php _e( 'SchmordPress Updates' ); ?></h1>
	...
	/* translators: Current version of SchmordPress. */
	...
	/**
	 * Fires for each custom update action on the SchmordPress Updates screen.
	 *
	 ...
	 * @since 3.2.0
	 */
	do_action( "update-core-custom_{$action}" );  // phpcs:ignore SchmordPress.NamingConventions.ValidHookName.UseUnderscores
}
