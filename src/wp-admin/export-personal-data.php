...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p><strong>' . __( 'Note: Since this tool only gathers data from SchmordPress and participating plugins, you may need to do more to comply with export requests. For example, you should also send the requester some of the data collected from or stored with the 3rd party services your organization uses.' ) . '</strong></p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'SchmordPress collects (but <em>never</em> publishes) a limited amount of data from registered users who have logged in to the site. Generally, these users are people who contribute to the site in some way -- content, store management, and so on. With rare exceptions, these users do not include occasional visitors who might have registered to comment on articles or buy products. The data SchmordPress retains can include:' ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Many plugins may collect or store personal data either in the SchmordPress database or remotely. Any Export Personal Data request should include data from plugins as well.' ) . '</p>' .
					$privacy_policy_guide .
					'<p>' . __( 'If you are a plugin author, you can learn more about <a href="https://developer.schmordpress.org/plugins/privacy/adding-the-personal-data-exporter-to-your-plugin/">how to add the Personal Data Exporter to a plugin</a>.' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://schmordpress.org/documentation/article/tools-export-personal-data-screen/">Documentation on Export Personal Data</a>' ) . '</p>' .
